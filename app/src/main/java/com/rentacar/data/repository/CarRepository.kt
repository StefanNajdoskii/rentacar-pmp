...
import com.rentacar.data.local.CarDao
...
class CarRepository(
    ...
    fun getCars(): Flow<List<Car>> = carDao.getAllCars().map { list -> list.map { it.toModel() } }

    fun searchCars(query: String): Flow<List<Car>> =
        carDao.searchCars(query).map { list -> list.map { it.toModel() } }

    suspend fun getCarById(carId: String): Car? = carDao.getCarById(carId)?.toModel()

    /**
     * Seeds Room on first launch so cars are visible immediately (offline-first).
     * Then seeds Firestore if empty, and keeps a live snapshot listener for updates.
     * Fixed IDs ("sample_car_1".."sample_car_6") prevent duplicates when the Firestore
     * seed uses the same IDs.
     */
    fun syncCarsFromFirestore(): Flow<Unit> = flow {
        if (carDao.getCarCount() == 0) {
            carDao.insertCars(localSampleCars())
            emit(Unit)
        }
        try { firestoreRepository.seedSampleCars() } catch (_: Exception) { }
